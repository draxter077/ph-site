...
export default function button(data){
    ...
    button.addEventListener(
        ...
        async function a(e){
            ...
            let password = e.target.parentElement.children[0].children[2].children[1].value
            ...
            if(name != data.name || email.toLowerCase() != data.email || (password != data.password && password != "")){
                if(password == ""){password = data.password}
                await axios.post(apiURL + "/client/changeClientData", {name:name, email:email.toLowerCase(), password:password})
